...
class Shooter:
    ...
    def update(self, mouse_pos: tuple[int, int]) -> None:
        ...
        if (
            self.current_bubble is not None
            and self.current_bubble.state == BubbleState.FIXED
        ):
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        pygame.draw.circle(
            screen, (100, 100, 100), (int(self.x), int(self.y)), self.bubble_size // 2
        )

        # Draw shooter barrel
        end_x = self.x + self.bubble_size * math.cos(self.angle)
        end_y = self.y - self.bubble_size * math.sin(
            self.angle
        )  # Subtract for upward direction
        pygame.draw.line(
            screen,
            (150, 150, 150),
